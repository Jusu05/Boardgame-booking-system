            ...
            for _ in range(random.randint(1, 5)):
                conn.execute("""
                    ...
                    SELECT
                        ...
                        ?, ?, ?
                    FROM boardgames b
                """, (i, i, f"{random.choices(LETTERS, k=12)}.png",
                    "image/png", random_png_bytes(100, 100)
                    )
                )
    ...
    for _ in range(n):
        conn.execute("""
            ...
            VALUES
                (?, ?, ?, ?)
        """, (
            random.randint(0, users_num),
            random.randint(0, boardgames_num),
            random.randint(1, 5),
            random.randint(0, 5)
        ))
